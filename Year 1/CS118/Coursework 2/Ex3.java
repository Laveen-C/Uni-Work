 ...
 * 
 * In this exercise, I decided to extend my Ex1.java code to implement a version of Tremaux's algorithm. This is because it lead on fairly easily 
 * from my Exercise 1 code, meaning that I could retain most of the original design of my Exercise 1 code whilst making some modifications to modify it.
 * 
 * The reason the original code for Ex1, as well as the previous code for Ex2, did not work is because they both attempt to backtrack in a loop, which 
 * takes it repeatedly across the 4 corners in the loop. The code I have implemented now uses the idea of turning around and returning when a given junction 
 * has already been visited beforehand, meaning it effectively transforms a maze with loops into a maze that is connected, since whenever we find a path 
 * that would close a loop, we treat it as a dead end by turning around and returning. 
 * 
 * When designing my code, I chose to check for junctions by checking for the nonwallExits, but when checking for potential exits from a junction I look for 
 * the number of passage exits from the junction. This leads to the problem of the robot being able to trap itself in a junction with all exits being beenbefore 
 * exits. To work around this, I treat this case separately; if the robot manages to get itself into this situation, it will backtrack out of that junction.
 */
